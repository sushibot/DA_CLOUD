...
export function VisualizerView({ onClose, expanded, analyserLeftRef, audioRef }: Props) {
  ...
  function togglePlayPause() {
    const audio = audioRef.current
    if (!audio) return
    if (status === 'playing') { audio.pause(); dispatch({ type: 'PAUSE' }) }
    else { audio.play(); dispatch({ type: 'PLAY' }) }
  }
  ...
  return (
    <div className={`relative h-full w-full bg-gray-950 transition-opacity duration-700 ease-in-out ${expanded ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="absolute inset-x-0 top-0 flex items-center justify-between px-6 pt-4">
        <button onClick={onClose} className="text-gray-400 hover:text-white cursor-pointer" title="Close">
      ...
      <div className="sm:hidden absolute inset-x-0 bottom-0 px-6 pb-10 pt-6 flex flex-col gap-4 bg-gradient-to-t from-gray-950/95 to-transparent">
        {/* Track name */}
