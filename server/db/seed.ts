...
const albumReleaseYear = process.argv[3];

if (!albumName) {
	console.error('Usage: pnpm seed <album-name> <album-release-year>')
	process.exit(1)
}
...
async function seed() {
	...
	if (albumName) {
		releaseYear = Number(albumReleaseYear);
	}
	...
	const keys = await listAudioKeys(`Music/albums/${albumName}`)
